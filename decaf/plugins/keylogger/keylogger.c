...
{
	uint32_t params_stack[7];
	DECAF_read_mem(NULL, cpu_single_env->regs[R_ESP], 7*4, params_stack);

	uint8_t* taint_flag = (uint8_t*)malloc(sizeof(uint8_t)*params_stack[3]);
	taintcheck_check_virtmem(params_stack[2], params_stack[3], taint_flag);

	int taint_bytes = 0, i;
	for(i == 0; i < params_stack[3]; i++) if(taint_flag[i]) taint_bytes += 1;
	free(taint_flag);
	...
	if(taint_bytes) fprintf(keylogger_log, "Process %s in function sendto: send %d tainted bytes!\n",name, taint_bytes);
}
...
{
	uint32_t  params_stack[5];
	DECAF_read_mem(NULL, cpu_single_env->regs[R_ESP], 5*4, params_stack);
	...
	if(taint_bytes) fprintf(keylogger_log, "Process %s in function send: send %d tainted bytes!\n",name, taint_bytes);
}
...
{
	uint32_t params_stack[6];
	DECAF_read_mem(NULL, cpu_single_env->regs[R_ESP], 6*4, params_stack);

	uint8_t* taint_flag = (uint8_t*)malloc(sizeof(uint8_t)*params_stack[4]);
	taintcheck_check_virtmem(params_stack[2], params_stack[4], taint_flag);

	int taint_bytes = 0, i;
	for(i = 0; i < params_stack[4]; i++) if(taint_flag[i]) taint_flag += 1;
	free(taint_flag);
	...
	if(taint_bytes) fprintf(keylogger_log, "Process %s WriteFile: write %d bytes tainted memory!\n", name, taint_bytes);
}

static void createproc_callback(VMI_Callback_Params* params)
{
	DECAF_printf("createproc: %s\n", params->cp.name);
