        ...
        for(middle_index = 0; middle_index < taint_memory_page_table_root_size; ++middle_index)
        {
            ...
            if(middle_node)
            {
                for(leaf_index = 0; leaf_index < (2 << BITPAGE_MIDDLE_BITS); ++leaf_index)
                {
                    ...
                    if(leaf_node)
                    {
                        for(bitmap_index = 0; bitmap_index < (2 << BITPAGE_LEAF_BITS); ++bitmap_index)
                        {  
                            if(*(leaf_node->bitmap + bitmap_index))
                            {
                                if(tf.mLen < SIZE)
                                {
                                    tf.mTaintInfo[tf.mLen].mMiddleIndex = middle_index;
                                    tf.mTaintInfo[tf.mLen].mLeafIndex = leaf_index;
                                    tf.mTaintInfo[tf.mLen].mBitmapIndex = bitmap_index;
                                    ++tf.mLen;
                                }//end if(tf.mLen < SIZE
                                else
                                {
                        ...
                        }//end for(bitmap_index = 0...
                    }//end if(leaf_node)  
                }//end for(leaf_index = 0;...
            }//end if(middle_node) 
        }//end for(middle_index = 0; ...   
    }//end if(taint_memory_page_table)
}

int set_taint_info(const uint32_t middle_index, const uint32_t leaf_index, const uint32_t bitmap_index)
{
    tbitpage_leaf_t *leaf_node = NULL;
    tbitpage_middle_t *middle_node = NULL;

    if(!taint_tracking_enabled || !taint_memory_page_table)
    {
        return 0;
    }
    middle_node = taint_memory_page_table[middle_index];
    if(!middle_node)
    {
        taint_memory_page_table[middle_index] = fetch_middle_node_from_pool();
    }

    leaf_node = taint_memory_page_table[middle_index]->leaf[leaf_index];

    if(!leaf_node)
    {
        taint_memory_page_table[middle_index]->leaf[leaf_index] = fetch_leaf_node_from_pool();
        leaf_node = taint_memory_page_table[middle_index]->leaf[leaf_index];
    }    
    
    leaf_node->bitmap[bitmap_index] = 0xff;
    return 1;
}
...
#ifdef ZK
    printf("do_enable_tainting_internal.\n");
#endif
    //DECAF_start_vm();
